...
export default function CopilotPanel({ events, busCount }: Props) {
  ...
  const [showKeyInput, setShowKeyInput] = useState(!import.meta.env.VITE_GROQ_API_KEY);
  ...
  const sendMessage = async (text: string) => {
    ...
    if (!apiKey) {
      setMessages(p => [...p, {
        role: 'ai',
        text: 'Groq API key not configured. Enter your key below to enable AI responses. Get a free key at console.groq.com',
      }]);
      setLoading(false);
    ...
    try {
      ...
      setMessages(p => [...p, {
        role: 'ai',
        text: `API error: ${msg.slice(0, 120)}. Check your Groq API key.`,
      }]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="flex flex-col h-full" style={{ maxHeight: '100%' }}>
      ...
      {showKeyInput && (
        <div className="flex-shrink-0 mb-3 p-3 rounded-xl"
          style={{ background:'rgba(249,115,22,0.06)', border:'1px solid rgba(249,115,22,0.2)' }}>
          <p className="text-xs font-mono mb-2" style={{ color:'#f97316' }}>
            Enter Groq API key to enable AI responses (free at console.groq.com):
          </p>
          <div className="flex gap-2">
            <input type="password" value={apiKey}
              onChange={e => setApiKey(e.target.value)}
              placeholder="gsk_..."
              className="flex-1 text-xs px-3 py-2 rounded-lg font-mono focus:outline-none"
              style={{ background:'#060e20', border:'1px solid #2d3449', color:'#dae2fd' }}
            />
            <button onClick={() => setShowKeyInput(false)}
              className="px-3 py-2 rounded-lg text-xs font-mono transition-all"
              style={{ background:'#f97316', color:'#1a0800' }}>
              Save
            </button>
          </div>
        </div>
